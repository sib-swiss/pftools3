...
static inline void * sre_malloc(char *file, int line, size_t size)
{
  void *ptr;
...
static inline void * sre_realloc(char *file, int line, void *p, size_t size)
{
  void *ptr;
...
ExtremeValueFitHistogram(struct histogram_s *h, const int censor, int lowscore, float high_hint) 
{
  ...
  for (iteration = 0; iteration < 100; iteration++)
	...
	{
	  x[sc-lowbound] = (float) sc + 0.5f; /* crude, but tests OK */
